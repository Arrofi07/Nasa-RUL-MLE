...
class TestSelectFeatures:
    ...
    def test_threshold_zero_returns_all_features(self, feature_train_df):
        ...
        feat_cols = [c for c in feature_train_df.columns if c not in ["engine_id", "cycle", "rul"]]
...
class TestAddRollingFeatures:
    ...
    def test_rolling_columns_created(self, feature_train_df, feature_test_df, feat_cols):
...
class TestAddDifferenceFeatures:
    ...
    def test_first_cycle_diff_is_zero(self, feature_train_df, feature_test_df, feat_cols):
    ...
    def test_diff_respects_engine_boundary(self, feat_cols):
        ...
        val = train[(train["engine_id"] == 2) & (train["cycle"] == 1)][f"{feat_cols[0]}_diff"].values[0]
