...
class TestLoadAndPreprocessData:
    ...
    def test_train_columns(self, raw_dir, tmp_path):
        ...
        assert list(train.columns) == expected, f"Unexpected columns: {train.columns.tolist()}"
    ...
    def test_rul_not_clipped_when_disabled(self, tmp_path):
        ...
        for eid in range(1, n_engines + 1):
            for c in range(1, n_cycles + 1):
                row = [eid, c] + rng.uniform(-1, 1, 3).tolist() + rng.uniform(400, 700, 21).tolist()
                rows.append(row)
...
class TestPreprocessDataIntegration:
    ...
    def test_train_test_same_feature_columns(self, processed_dir):
        ...
        assert set(train_feat) == set(test_feat), "Train and test feature columns differ"